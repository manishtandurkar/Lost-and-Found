...
import com.example.lostandfound.services.MatchingService;
...
public class ReportFoundActivity extends AppCompatActivity {
    ...
    private void observeViewModel() {
        ...
        reportViewModel.getPhotoUploadFailed().observe(this, failed -> {
            if (failed != null && failed) {
                Snackbar.make(btnSubmit, getString(R.string.photo_upload_failed),
                        Snackbar.LENGTH_LONG).show();
            }
        });
    ...
    private void triggerMatchingService(String itemId) {
        Intent serviceIntent = new Intent(this, MatchingService.class);
        serviceIntent.putExtra(Constants.KEY_NEW_ITEM_ID, itemId);
        serviceIntent.putExtra(Constants.KEY_NEW_ITEM_TYPE, Constants.ITEM_TYPE_FOUND);
        serviceIntent.putExtra(Constants.KEY_NEW_ITEM_CATEGORY,
                actvCategory.getText().toString().trim());
        serviceIntent.putExtra(Constants.KEY_NEW_ITEM_TITLE,
                etTitle.getText() != null ? etTitle.getText().toString().trim() : "");
        serviceIntent.putExtra(Constants.KEY_NEW_ITEM_POSTED_BY, sessionManager.getUserId());
    ...
    private void showPhotoOptions() {
        new MaterialAlertDialogBuilder(this)
                ...
                .setItems(new CharSequence[]{
                    ...
                    else if (which == 1) checkStoragePermissionAndPick();
                })
                .show();
    }
